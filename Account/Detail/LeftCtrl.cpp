...
IMPLEMENT_DYNCREATE(CLeftCtrl, CWnd)
...
BOOL CLeftCtrl::AddPage(CRuntimeClass* pClass,UINT nIDTemplate, LPCTSTR sText, UINT IconID)
{
	...
	if(pDlg != NULL)
	{
		...
		{
			CLeftCtrl::CPageItem *pItem = AddPage(pDlg, sText, IconID);
			if(pItem)
			{
...
///////取得需要占用的宽度/////
UINT CLeftCtrl::CPageItem::GetAreaWidth(CDC *pDC)
{
...
void CLeftCtrl::SetActivePage(int nIndex)
{
...
void CLeftCtrl::OnSizing(UINT fwSide, LPRECT pRect) 
{
...
void CLeftCtrl::DrawFrame(CDC *pDC)
{
	CRect rect;
	GetClientRect(&rect);
	switch(m_nBorder)
...
void CLeftCtrl::SetStyle(UINT style)
{
	m_nStyle = style;
}

UINT CLeftCtrl::GetStyle()
{
	return m_nStyle;
}
...
void CLeftCtrl::SetStyleAnimate(BOOL bEnable)
{
	UINT style = GetStyle();
	if(bEnable)
		style |= TCS_ANIMATE;
...
void CLeftCtrl::SetStyleDirection(int Direction)
{
	UINT style = GetStyle();
	switch(Direction)
	{
...
void CLeftCtrl::SetStyleMonoSpace(BOOL bEnable)
{
	UINT style = GetStyle();
	if(bEnable)
		style |= TCS_MONOSPACE;
...
BOOL CLeftCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
...
int CLeftCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CString str;
	str.Format(_T("宋体"));
	memccpy(logFont.lfFaceName,str,NULL,str.GetLength());
	/*CString  str;
	str.Format(_T("宋体"));
	int iLength ;    
	iLength = WideCharToMultiByte(CP_ACP, 0, str, -1, NULL, 0, NULL, NULL);     
	WideCharToMultiByte(CP_ACP, 0, str, -1, logFont.lfFaceName, iLength, NULL, NULL); */
